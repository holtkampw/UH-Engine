...
namespace BoxCollider
{
    ...
    public class CollisionMesh
    {
        ...
        public CollisionMesh(Model model, uint subdivLevel)
            : this(model, subdivLevel, Vector3.Zero) { }

        public CollisionMesh(Model model, uint subdivLevel, Vector3 position)
        {
            ...
            foreach (ModelMesh mesh in model.Meshes)
            {
                ...
                foreach (ModelMeshPart part in mesh.MeshParts)
                {
                    ...
                    part.VertexBuffer.GetData<CustomVertex>(partVertices);

                    for (int i = 0; i < vertexCount; i++)
                    {
                        vertices[verticesLength + i] =
                            Vector3.Transform(Vector3.Add(partVertices[i].Position, position), meshTransform);
                    }
